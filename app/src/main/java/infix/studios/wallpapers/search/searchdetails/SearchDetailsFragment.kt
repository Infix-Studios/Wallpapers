        ...
        when(item.itemId) {
            R.id.favorite_menu_icon -> {
                if (currentMenuIcon.icon.constantState?.equals(
                    ...
                    currentMenuIcon.icon = getDrawable(requireContext(), R.drawable.ic_white_favorite_24)
                    } else {
                    //Toast.makeText(context, "Removed to favorite", Toast.LENGTH_SHORT).show()
                    currentMenuIcon.icon = getDrawable(requireContext(), R.drawable.ic_unliked_favorite_24)
                }
            }
        }
